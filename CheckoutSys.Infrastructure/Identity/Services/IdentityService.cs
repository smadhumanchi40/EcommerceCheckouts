using CheckoutSys.Application.DTOs.Identity;
using CheckoutSys.Application.DTOs.Mail;
using CheckoutSys.Application.DTOs.Settings;
using CheckoutSys.Application.Enums;
using CheckoutSys.Application.Exceptions;
using CheckoutSys.Application.Interfaces;
...
namespace CheckoutSys.Infrastructure.Identity.Services
{
        ...
        public async Task<Result<string>> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            if (userWithSameEmail == null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _mailService.SendAsync(new MailRequest() { From = "mail@code.com", To = user.Email, Body = $"Please confirm your account by <a href='{verificationUri}'>clicking here</a>.", Subject = "Confirm Registration" });
